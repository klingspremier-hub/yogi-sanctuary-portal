...
const InteractiveCard = ({ children, delay = 0, isInView }: InteractiveCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="bg-card/80 backdrop-blur-sm border border-primary/10 p-6 md:p-8 group cursor-pointer hover:border-primary/30 md:hover:-translate-y-1 transition-all duration-500 shadow-sm hover:shadow-lg"
      style={
        {
          "--mouse-x": `${mousePos.x}%`,
          "--mouse-y": `${mousePos.y}%`,
...
const IntentionSection = () => {
  ...
  return (
    <section
      ...
      className="py-16 md:py-40 px-4 md:px-6 luxury-gradient-warm relative overflow-hidden"
    >
      ...
      <div ref={ref} className="max-w-5xl mx-auto text-center relative z-10">
        ...
        >
          <span className="text-sm uppercase tracking-editorial font-bold mb-4 md:mb-6 block text-gold">
            The Vision
          </span>
          <h2
            id="intention-heading"
            className="font-serif text-2xl sm:text-3xl md:text-5xl lg:text-6xl mb-8 md:mb-12 leading-snug tracking-tight text-primary"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 text-left">
            ...
            <InteractiveCard delay={0.25} isInView={isInView}>
              ...
              <h3 className="font-serif text-xl md:text-2xl text-primary mb-2 md:mb-3">
